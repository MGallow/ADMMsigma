...
where $0 \leq \alpha \leq 1$, $\lambda > 0$, $0 < \eta < 2$, $\left\|\cdot \right\|_{F}^{2}$ is the Frobenius norm and we define $\left\|A \right\|_{1} = \sum_{i, j} \left| A_{ij} \right|$. This penalty is closely related to the elastic-net penalty explored by Hui Zou and Trevor Hastie [@zou2005regularization]. Clearly, when $\alpha = 0$ this reduces to a ridge-type penalty and when $\alpha = 1$ this reduces to a lasso-type penalty.
...
microbenchmark(ADMMsigma(X, lam = 10^seq(-8, 8, 0.1), alpha = 1, crit = "loglik"), times = 5)

# benchmark ADMMsigma CV
microbenchmark(ADMMsigma(X, lam = 10^seq(-8, 8, 0.1), alpha = 1), times = 5)
