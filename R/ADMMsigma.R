...
ADMMsigma = function(X = NULL, S = NULL, nlam = 10, lam.min.ratio = 0.01, lam = NULL, alpha = seq(0, 1, 0.2), diagonal = FALSE, 
    path = FALSE, rho = 2, mu = 10, tau.inc = 2, tau.dec = 2, crit = c("ADMM", "loglik"), tol.abs = 1e-04, tol.rel = 1e-04, 
    maxit = 10000, adjmaxit = NULL, K = 5, crit.cv = c("loglik", "AIC", "BIC"), start = c("warm", "cold"), cores = 1, trace = c("progress", 
        "print", "none")) {
    ...
    if (!(all(sapply(c(rho, mu, tau.inc, tau.dec, tol.abs, tol.rel, maxit, adjmaxit, K, cores, nlam, lam.min.ratio), length) <= 
        1))) {
    ...
    if ((length(lam) > 1 || length(alpha) > 1) & (!is.null(X) || path)) {
        ...
        if (cores > 1) {
            ...
            # execute CVP_ADMM
            ADMM = CVP_ADMM(X = X, lam = lam, alpha = alpha, diagonal = diagonal, rho = rho, mu = mu, tau.inc = tau.inc, 
                tau.dec = tau.dec, crit = crit, tol.abs = tol.abs, tol.rel = tol.rel, maxit = maxit, adjmaxit = adjmaxit, 
            ...
            ADMM = CV_ADMMc(X = X, S = S, lam = lam, alpha = alpha, diagonal = diagonal, path = path, rho = rho, mu = mu, 
                tau_inc = tau.inc, tau_dec = tau.dec, crit = crit, tol_abs = tol.abs, tol_rel = tol.rel, maxit = maxit, adjmaxit = adjmaxit, 
        ...
        ADMM = ADMMc(S = S, initOmega = init, initZ2 = init, initY = init, lam = ADMM$lam, alpha = ADMM$alpha, diagonal = diagonal, 
            rho = rho, mu = mu, tau_inc = tau.inc, tau_dec = tau.dec, crit = crit, tol_abs = tol.abs, tol_rel = tol.rel, 
            maxit = maxit)
        ...
        ADMM = ADMMc(S = S, initOmega = init, initZ2 = init, initY = init, lam = lam, alpha = alpha, diagonal = diagonal, 
            rho = rho, mu = mu, tau_inc = tau.inc, tau_dec = tau.dec, crit = crit, tol_abs = tol.abs, tol_rel = tol.rel, 
            maxit = maxit)
    ...
    loglik = (-n/2) * (sum(ADMM$Omega * S) - determinant(ADMM$Omega, logarithm = TRUE)$modulus[1] + ADMM$lam * ((1 - ADMM$alpha)/2 * 
    ...
    returns = list(Call = call, Iterations = ADMM$Iterations, Tuning = tuning, Lambdas = lam, Alphas = alpha, maxit = maxit, 
        Omega = ADMM$Omega, Sigma = qr.solve(ADMM$Omega), Path = Path, Z = ADMM$Z2, Y = ADMM$Y, rho = ADMM$rho, Loglik = loglik, 
...
plot.ADMM = function(x, type = c("line", "heatmap"), footnote = TRUE, ...) {
    ...
    if (type == "line") {
        ...
        if (length(x$Alphas) > 1) {
            ...
            # produce line graph
            graph = ggplot(summarise(group_by(cv, lam, alpha), Means = mean(Errors)), aes(log10(lam), Means, color = as.factor(alpha))) + 
                theme_minimal() + geom_line() + labs(title = "Cross-Validation Errors", color = "alpha", y = "Average Error") + 
    ...
    if (footnote) {
        ...
        graph + labs(caption = paste("**Optimal: log10(lam) = ", round(x$Tuning[1], 3), ", alpha = ", round(x$Tuning[2], 
            3), sep = ""))
