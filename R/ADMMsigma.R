...
ADMMsigma = function(X = NULL, S = NULL, 
    ...
    if ((length(lam) > 1 || length(alpha) > 
        ...
        # compute final estimate at best
        # tuning parameters
    ...
    # compute gradient
    grad = S - qr.solve(ADMM$Omega) + 
        ADMM$lam * (1 - ADMM$alpha) * 
            ADMM$Omega + ADMM$lam * ADMM$alpha * 
        sign(ADMM$Omega)
    
    # return values
    tuning = matrix(c(log10(ADMM$lam), 
        ADMM$alpha), ncol = 2)
    colnames(tuning) = c("log10(lam)", 
        "alpha")
    returns = list(Iterations = ADMM$Iterations, 
        Tuning = tuning, Lambdas = lam, 
        Alphas = alpha, maxit = maxit, 
...
print.ADMMsigma = function(x, ...) {
    ...
    cat("\nTuning parameters:\n")
    print.default(round(x$Tuning, 3), 
