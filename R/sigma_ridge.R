...
# we define the ridge penalized covariance
# matrix function
sigma_ridge = function(S, lam) {
    ...
    # compute omega hat for lambda (zero gradient
    # equation)
...
CV_sigma_ridge = function(X, lam, ind = NULL, 
    K = 5, quiet = TRUE) {
    ...
    for (k in 1:K) {
        ...
        for (i in 1:length(lam)) {
            ...
            ## loglikelihood value
            cv.loss[i, k] = sum(omega * sigma.valid) - 
    ...
    omega = sigma_ridge(S = sample.covariance, 
        lam = best.lam)
    
    
    return(list(omega = omega, best.lam = best.lam, 
