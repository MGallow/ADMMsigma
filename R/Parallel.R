...
CVP_ADMM = function(X = NULL, lam = 10^seq(-2, 2, 0.2), alpha = seq(0, 1, 0.2), diagonal = FALSE, rho = 2, mu = 10, tau.inc = 2, 
    tau.dec = 2, crit = c("ADMM", "loglik"), tol.abs = 1e-04, tol.rel = 1e-04, maxit = 1000, adjmaxit = NULL, K = 5, crit.cv = c("loglik", 
    ...
    CV = foreach(k = 1:K, .packages = "ADMMsigma", .inorder = FALSE) %dopar% {
        ...
        CVP_ADMMc(n = nrow(X.valid), S_train = S.train, S_valid = S.valid, lam = lam, alpha = alpha, diagonal = diagonal, 
            rho = rho, mu = mu, tau_inc = tau.inc, tau_dec = tau.dec, crit = crit, tol_abs = tol.abs, tol_rel = tol.rel, 
            maxit = maxit, adjmaxit = adjmaxit, crit_cv = crit.cv, start = start, trace = trace)
...
CVP_RIDGE = function(X = NULL, lam = 10^seq(-2, 2, 0.1), K = 5, cores = 1, trace = c("none", "progress", "print")) {
    ...
    CV = foreach(k = 1:K, .packages = "ADMMsigma", .combine = "cbind", .inorder = FALSE) %dopar% {
        ...
        CVP_RIDGEc(n = nrow(X.valid), S_train = S.train, S_valid = S.valid, lam = lam, trace = trace)
